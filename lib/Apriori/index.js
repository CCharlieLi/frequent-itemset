...
const isInclude = require(path.resolve( __dirname, './isInclude'));

module.exports = (arrs, minSupport = 0.5) => {
  ...
  const supportCal = (L) => {
    L.forEach(each => {
      ...
      if (scount / originalArrsLength < minSupport)
        failedItemsMap.set(each, scount);
      else
        succeedItemsMap.set(each, scount);
    });
